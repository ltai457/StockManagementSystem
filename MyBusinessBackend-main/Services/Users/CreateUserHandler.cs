...
public class CreateUserHandler : ICreateUserHandler
{
    ...
    {
        if (await _service.UsernameExistsAsync(dto.Username)) return Result<UserDto>.Conflict($"Username '{dto.Username}' exists.");
        if (await _service.EmailExistsAsync(dto.Email)) return Result<UserDto>.Conflict($"Email '{dto.Email}' exists.");
        var u = await _service.CreateAsync(dto.Username, dto.Email, dto.Password, dto.Role, dto.FirstName, dto.LastName);
        return u is null ? Result<UserDto>.Fail("Failed to create user.") : Result<UserDto>.Created(UserMapper.ToDto(u), "GetById", new { id = u.Id });
    }
}
