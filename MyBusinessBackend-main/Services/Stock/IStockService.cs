...
public interface IStockService
{
    ...
    Task<bool> UpdateStockAsync(Guid radiatorId, string warehouseCode, int quantity, Guid? updatedBy = null, string? reason = null);
    ...
    Task<(bool Success, string? Error, int SuccessCount, int ErrorCount, List<(Guid RadiatorId, string WarehouseCode, string Error)> Errors)> BulkUpdateStockAsync(List<StockUpdateItemDto> updates, Guid? updatedBy = null, string? reason = null);
    Task<(Warehouse Warehouse, List<StockLevel> StockLevels)?> GetWarehouseStockDataAsync(string warehouseCode);
    Task<(bool Success, string? Error, Guid RadiatorId, string WarehouseCode, int OldQuantity, int NewQuantity, string? Reason)> AdjustStockAsync(Guid radiatorId, string warehouseCode, int newQuantity, string? reason, Guid? updatedBy = null);
